...
### Stop 훅
Claude가 작업을 종료하려 할 때 자동으로 아래 기준을 점검한다:
1. 요청된 기능/수정이 코드에 반영됐는가
2. 브라우저 또는 테스트로 동작이 확인됐는가
3. 필요한 경우 GitHub push가 완료됐는가

세 가지를 모두 충족한 경우에만 종료. 하나라도 미완료면 계속 작업.

### 스킬
- `/deploy`: 변경사항 커밋 → push → CLAUDE.md 세션 기록 업데이트

### 규칙
- `.env` 파일은 API 키·비밀번호 등 실제 보안 민감 정보가 있을 때만 만든다
- 로컬 경로(EXCEL_PATH 등) 같은 단순 설정은 코드에 직접 두고 CLAUDE.md에 메모한다
- AI 지시사항(CLAUDE.md, settings.json, 스킬)은 모두 GitHub에 공유한다

...
전달된 실적 데이터(이미 계산된 지표)를 읽고, 팀장·경영진이 5분 안에 의사결정할 수 있는
분석문을 작성하세요.
...
2. 적자는 절대 건수만으로 말하지 마세요. "적자매출비중_pct"(매출 가중)를 반드시 함께
   제시하세요. 건수 비중과 매출 비중이 다르면 그 자체가 중요한 발견입니다.
3. 필요배수가 2 이상인 파트는 "연말 몰아치기 리스크"로 명시하세요.
4. 손익률/원가율은 이미 금액 가중평균으로 계산되어 있습니다 — 다시 계산하지 말고 그대로 쓰세요.
5. 숫자는 전달된 값만 쓰세요. 스스로 계산하지 마세요(나눗셈·비율 재계산 금지).
6. 재무_코드충돌_건수가 0보다 크면 마지막에 짧게 "데이터 신뢰도: 코드충돌 N건 — 해당
   PPT 데이터는 한쪽이 덮어써졌을 수 있음"이라고 경고하세요.

## 서술 규칙
- 모든 발견은 "무슨 일이 일어났나 → 왜 중요한가 → 무엇을 해야 하나" 순서로.
- 비교 기준 없는 숫자 금지 (반드시 계획/전기/기준선 중 하나와 비교).
- 모호한 헤지("~일 수 있습니다") 금지 — 확인이 필요하면 "확인 필요: X"로 대상을 특정.
- 숫자 없는 문장 금지.

## 출력 구조 (총 900자 이내, 마크다운 간단히 사용 가능)
1. **한 줄 결론** — 연말 착지 전망 + 최대 리스크
2. **착지 전망** — 계획/누계/안분기대치/연간추정, 안분 달성률로 페이스 판정
3. **페이스 리스크** — 필요배수 2 이상 파트, 잔여 기간 부담
4. **수익성 진단** — 파트별 손익률/원가율, 적자매출비중 기준 최악 파트 지목
5. **무엇이 돈이 되는가** — 사업구분/교육형태 상위·하위 3개, 규모×수익성 해석
6. **즉시 조치 필요** — 최대 5개, 금액 영향 순, [대상] 사실→원인추정→권고 형식
"""
...
## ⚠️ 반드시 먼저 할 일 — 데이터 신뢰도 판정
전달된 "항목별_기재율"의 "판정" 필드를 그대로 신뢰하세요(이미 계산됨):
- "집계불가": 이 항목은 순위·비교·추세를 절대 말하지 마세요. 표본이 대표성 없습니다.
- "참고용": 한쪽만 기재됨 — 참고 수치로만, 확정적 판단 금지.
- "분석가능": 정상 분석 대상.
"집계불가" 항목이 있으면 분석 맨 위에 "N개 항목은 기재율 부족으로 집계 불가" 경고를 쓰세요.

## 반드시 지킬 규칙
1. 실적 미기재를 "실적 0"으로 단정하지 마세요 — "미기재"와 "0 입력"은 다릅니다.
2. 코드충돌_건수가 0보다 크면 데이터 신뢰도 경고에 포함하세요(서로 다른 PPT가 같은
   코드를 공유해 한쪽 데이터가 덮어써져 사라졌을 수 있음).
3. 숫자는 전달된 값만 쓰고 스스로 계산하지 마세요.
4. 숫자 없는 문장 금지, 모호한 헤지 금지.

## 출력 구조 (총 700자 이내)
1. **데이터 신뢰도 판정** — 표 또는 목록, 집계불가 항목 경고
2. **분석 가능한 항목 진단** — "분석가능"/"참고용" 항목만, 기재율과 함께
3. **파트별 참여 현황** — 행수 기준, 미기재 파트 명시
4. **데이터 품질 경고** — 코드중복 등, 해당 파트에 수정 요청

...
def main():
    ...
    blocks = [
        ...
        ("row", ["05_대시보드계산", "화면의 카드·차트·표가 각각 어느 컬럼에서 어떻게 나오는지 + '왜 이 데이터·이 방식인가' 열"]),
        ("row", ["06_파트별실적검산", "파트별 실적표를 독립 계산으로 재검산한 결과"]),
        ("row", ["07_타당성검토", "계산이 '맞는지'를 넘어 '이렇게 하는 게 옳은지' 검토와 권고 (쟁점 13가지)"]),
        ("gap",),
    ...
    # 각 행: (구역, 항목, 값, 계산식, 설명, 왜 이 데이터·이 방식인가)
    dash = [
        ("KPI 카드", "매출/원가 계획",
         f"{E(drev['plan_initial'].sum()):.1f}억 / 원가 {E(dcost['plan_initial'].sum()):.1f}억",
         "SUM(V열) — 매출행 / 원가행 각각", "연초 세운 연간 목표. 달성률의 분모.",
         "이후 모든 '계획 대비' 지표(달성률·증감)의 기준선이라 흔들리지 않는 '최초 사업계획(V)'을 씀. "
         "중간 수정본은 목표 자체가 움직여 비교 잣대가 안 됨."),
        ("KPI 카드", "매출/원가 추정 실적",
         f"{E(drev['jun_check_total'].sum()):.1f}억 / 원가 {E(dcost['jun_check_total'].sum()):.1f}억",
         "SUM(BH열) — 매출행 / 원가행 각각", "8월 시점에서 다시 본 연간 전망(실적+남은달 추정). ★ 여기서 '원가'는 직접원가만.",
         "계획(V)이 '연간' 단위라, 같은 연간 축으로 비교하려면 누계(BI~BP)가 아니라 연간합(BH)을 써야 함. "
         "★ BH의 '원가'는 직접원가(BB)뿐 — 인건비·공통·관리비 미포함이라 손익 계산의 총원가와 다름."),
        ("KPI 카드", "매출이익(당해년도 추정)", f"{E(drev['profit_gross'].sum()):.1f}억",
         "SUM(BA열) — 매출행만", "매출 − 직접원가. 인건비·공통원가·관리비 빼기 전 1차 이익.",
         "화면의 '매출 − 원가' 암산에 실제로 맞아떨어지는 값이 경상손익이 아니라 이것(BA). "
         "아직 안 뺀 원가가 있다는 걸 드러내려고 경상손익과 나란히 별도 카드로 둠."),
        ("KPI 카드", "경상손익(당해년도 추정)", f"{E(drev['operating_profit'].sum()):.1f}억",
         "SUM(BF열) — 매출행만", "모든 원가를 뺀 최종 손익. 카드 sub의 손익률은 가중평균.",
         "'결국 얼마 남았나'의 정답이라 최종 지표로 씀. 매출행 BF만 더해도 되는 이유는 "
         "원가행 BF가 295행 전부 0임을 전수 확인했기 때문."),
        ("KPI 카드", "매출/원가 누계 실적 (1~8월)",
         ...
         "'지금까지 실제로 얼마'를 보려면 발생분만 필요. AR열은 이름과 달리 연간 전체값(미래월 추정 포함)이라 "
         f"그대로 쓰면 {E(AR.sum())/E(sum08.sum()):.1f}배 부풀려짐 → 경과월 열만 다시 합산."),
        ("차트", "월별 실적 추이", "12개 막대 × 2계열",
         "매출=SUM(BI~BT) 매출행, 원가=SUM(BI~BT) 원가행, 월별로", "9월 이후는 추정치라 흐리게 표시.",
         "월별 추세를 보려면 연간합(BH)이 아니라 개별 월 열이 있어야 함. "
         "미래월을 흐리게 처리하는 건 그 값들이 실적이 아니라 추정이기 때문."),
        ("차트", "파트별 추정 매출/원가", "파트 7개 × 2계열",
         "매출=SUM(BH) 매출행, 원가=SUM(BH) 원가행, 파트별", "연간 추정 기준(누계 아님).",
         "파트 간 규모를 같은 잣대로 비교하려면 '연말이면 얼마'인 연간 추정(BH)이 기준. "
         "누계로 비교하면 아직 안 지난 기간만큼 전부 작게 보임."),
        ("차트", "프로젝트 합계 원가비율(도넛)", "4조각",
         "SUM(BB)/SUM(BC)/SUM(BD)/SUM(BE) — 매출행", "전체 합계의 구성비. 프로젝트별 비율의 평균이 아님.",
         "'우리 원가가 어디에 쓰였나(인건비 vs 공통 vs 관리비 비중)'를 답하려면 합계 기준이어야 함. "
         "프로젝트별 비율 단순평균은 '평균적인 프로젝트의 원가 구조'라 다른 질문(합계 67.5% vs 평균 56.7%). "
         "관리비(BE)까지 4조각인 이유 — 관리비가 전체 원가의 12%라 빼면 그만큼 그림이 어긋남."),
        ("차트", "파트별 계획 vs 추정 실적", "파트별 2계열",
         "계획=SUM(V) 매출행, 추정=SUM(BH) 매출행", "둘 다 연간 기준이라 같은 기간끼리 비교.",
         "목표(V)와 전망(BH) 모두 연간이라 8개월/12개월 기간 불일치 없이 바로 비교됨. "
         "여기서 누계(BI~BP)를 쓰면 '계획 대비 저조'가 착시로 생김."),
        ("차트", "파트별 달성 현황(진행바)", "파트별 %",
         "SUM(BI~BP) ÷ SUM(V) × 100 — 매출행", "★ 8개월 누계를 연간 계획과 비교 → 07 시트 참조.",
         "★ '얼마나 달성했나'를 8개월 실적 ÷ 연간 계획으로 보는데, 66.7%가 지난 시점에 44%로 나와 '미달'로 오독됨. "
         "같은 데이터의 연간 추정은 계획의 117%. 분자·분모 기간이 안 맞는 게 한계 — 07 시트 권고."),
        ("표", "파트별 실적 — 매출 계획", "파트별", "SUM(V) 매출행", "",
         "KPI 카드 계획과 같은 값을 파트로 쪼갠 것. 근거 동일(최초 사업계획 = 비교 기준선)."),
        ("표", "파트별 실적 — 누계매출/누계원가", "파트별", "SUM(BI~BP) 매출행 / 원가행", "",
         "실제 발생분만. AR열 대신 경과월 합을 쓰는 근거는 KPI 카드 '누계 실적'과 동일."),
        ("표", "파트별 실적 — 원가율", "파트별", "누계원가 ÷ 누계매출 × 100", "프론트에서 계산.",
         "누계끼리 나눠야 기간이 맞음(누계원가 ÷ 연간매출로 나누면 비율이 축소됨). 분모가 매출인 건 원가율의 정의."),
        ("표", "파트별 실적 — 추정 실적(연간)", "파트별", "SUM(BH) 매출행", "",
         "연간 전망. 계획(V)과 같은 연간 축이라 나란히 비교 가능."),
        ("표", "파트별 실적 — 경상손익", "파트별", "SUM(BF) 매출행", "음수면 빨간색 표시.",
         "모든 원가 차감 후 최종 손익. 매출행만 더해도 되는 근거는 KPI 카드 경상손익과 동일(원가행 BF=0)."),
        ("표", "파트별 실적 — 손익률", "파트별", "SUM(BF) ÷ SUM(BH) × 100  (가중평균)", "★ 단순평균 아님 — 07 시트 참조.",
         "★ 파트가 '실제로 얼마를 벌었나'는 총 경상손익 ÷ 총 매출이라는 정의 그대로. "
         "행별 손익률을 단순평균하면 금액 작은 프로젝트의 극단 비율에 휘둘려 부호까지 뒤집힘(⑤ 신사업 가중 −0.4% vs 단순 +0.6%)."),
        ("표", "프로젝트 상세", f"{n_rev+n_cost}행", "행 그대로 표시 (매출/원가 2행을 프로젝트 단위로 병합 표시)",
         "정식 코드는 코드만으로, placeholder는 코드+이름으로 묶음.",
         "집계가 아니라 원본 확인용이라 가공하지 않고 그대로 노출. 병합 키를 코드+이름으로 하면 "
         "이름 오타로 한 프로젝트가 쪼개지고, 코드만으로 하면 placeholder 코드를 공유한 남의 프로젝트가 섞임 → 하이브리드."),
    ]
    blocks = [
        ("title", "■ 화면의 숫자 하나하나가 어느 컬럼에서 어떻게 나오는가"),
        ("head", ["구역", "화면 항목", "현재 값", "계산식 (원본 컬럼 기준)", "설명", "왜 이 데이터·이 방식인가"]),
    ]
    for d in dash:
        fill = OK_FILL if d[3].startswith("SUM(BI~BP)") or "★" in d[4] or "★" in d[5] else None
    ...
    write_sheet(wb, "05_대시보드계산", blocks, [11, 26, 20, 40, 34, 66])
    ...
    blocks = [
        ...
        ("row", ["달성률을 '8개월 누계 ÷ 연간 계획'으로 볼 것인가",
                 "누계(1~8월) ÷ 연간계획 × 100  +  막대에 경과 기준선(66.7%) 표시",
                 "조치 완료 (기준선 추가)",
                 "8월 시점은 연간의 66.7%가 지난 때라 분자(8개월)/분모(연간) 기간이 안 맞아 44.3%로 낮게 나옴. "
                 "숫자 자체는 정확. 2026-09 화면에 '경과 기준선'을 추가해, 막대가 그 선을 넘으면 계획보다 앞선 것으로 "
                 "바로 읽히도록 함(달성률 값은 그대로 두고 오독만 방지). 참고로 같은 데이터 연간 추정은 계획의 117.2%."], OK_FILL),
        ...
        ("row", ["매출행·원가행을 K열 '구분'으로 나눠 따로 SUM 하는 이유",
                 "매출행만 / 원가행만 각각 집계",
                 "필수적이며 옳음",
                 "한 프로젝트는 시트에 매출행 1 + 원가행 1로 들어가는데 V·BH 등 같은 컬럼에 둘 다 값이 있음. "
                 "K열 조건 없이 파트로만 필터하면 매출+원가가 합쳐짐 — 실제로 ⑥ PM 매출계획이 "
                 "2,252,630(매출행)이 아니라 3,291,327(매출행+원가행)으로 잘못 나온 사례가 있었음."], OK_FILL),
        ("row", ["'추정 실적(BH)'과 '누계 실적(BI~BP)'을 둘 다 보여주는 이유",
                 "카드·표에 두 값을 나란히 노출",
                 "적절함",
                 "BH는 '연말이면 얼마'(계획과 비교용), BI~BP는 '지금까지 얼마'(진행 확인용)로 답하는 질문이 다름. "
                 "하나만 두면 '계획 대비 저조'(누계만) 또는 '이미 다 한 것처럼'(연간만) 오해가 생김."], OK_FILL),
        ("row", ["미래월(9~12월)을 집계에서 빼지 않고 흐리게만 표시하는 이유",
                 "BH·월별 열에 추정치 포함한 채로 집계, 화면에서 페이드",
                 "적절함",
                 "'연간 전망'을 보여주는 게 목적이라 추정치도 값의 일부. 다만 실적과 추정을 눈으로 구분하도록 "
                 "9월 이후 막대를 흐리게 처리. 누계 지표에서는 반대로 미래월을 완전히 제외(jun_actual 보정)."], OK_FILL),
        ("row", ["매출이익(BA)과 경상손익(BF)을 둘 다 카드로 두는 이유",
                 "두 이익 지표를 별도 카드로",
                 "적절함 (혼동 방지 목적)",
                 "'매출 376 − 원가 244 = 132'라는 화면 암산이 실제로는 매출이익(BA)이고 경상손익(BF, 14.3)이 아님. "
                 "'원가'로 표시되는 값이 직접원가만이라 생기는 착시라, 두 이익을 같이 보여줘 차이를 드러냄."], OK_FILL),
        ("row", ["파트가 파일명으로 안 잡히는 행을 폴더명으로 보정하는 게 맞나",
                 "파일명에 파트 키워드 없으면 상위 폴더명으로 판정",
                 "적절함",
                 "보정 안 하면 파트='-'로 빠져 파트별 합 ≠ 전체 합이 됨. 실측상 폴더 구조가 파트와 1:1이라 "
                 "폴더명 판정이 안전. (그래도 '-' 2건은 남아 필터에서 자동 포함되도록 별도 처리)"], OK_FILL),
        ("gap",),
        ("title", "■ 종합 결론"),
